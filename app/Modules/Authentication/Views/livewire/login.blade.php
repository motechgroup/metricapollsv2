    ...
    <div class="w-full max-w-md bg-white border border-gray-200 p-8 rounded-lg shadow-sm space-y-8 animate-fade-in" x-data="{ showPicker: false }">
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-md bg-blue-50 p-3 border border-blue-150">
                <p class="text-xxs text-blue-700 leading-relaxed font-semibold">
                    💡 SECURITY NOTICE: Panelist registrations and paid payouts are restricted to authenticated Google Accounts only to prevent fraud.
                </p>
            </div>
            ...
            <div x-show="showPicker" x-transition class="border border-gray-200 rounded-lg p-4 bg-gray-50 space-y-3" style="display: none;">
                ...
                <div class="space-y-2">
                    ...
                    <button wire:click="loginWithGoogle('admin@metricapolls.com', 'Metrica Admin')" class="w-full text-left bg-white border border-gray-150 p-2.5 rounded-md hover:bg-gray-100 transition flex items-center justify-between text-xs">
                        <div>
                            <span class="font-bold text-gray-900 block">Metrica Admin</span>
                            <span class="text-gray-500">admin@metricapolls.com</span>
                        </div>
                        <span class="text-xxs font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded uppercase">Super Admin</span>
                    </button>
                </div>
            </div>
        </div>

        <div class="relative border-t border-gray-200 my-6">
            <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 text-xxs font-bold text-gray-400 uppercase tracking-widest">Administrative Backdoor</span>
        </div>

        <!-- Administrative Email / Password Login (As requested, keeping as backdoor) -->
        <form wire:submit.prevent="login" class="space-y-4">
            <div>
                <label for="email" class="block text-xxs font-bold uppercase tracking-wider text-gray-400">Backdoor Email Address</label>
        ...
        <div class="mt-6 text-center border-t border-gray-150 pt-6">
            <p class="text-xs text-gray-500">
                Don't have a Metrica account? 
                <a href="{{ route('auth.register') }}" class="font-bold text-gray-900 hover:underline">Sign up via Google</a>
            </p>
        </div>
    </div>
</div>
