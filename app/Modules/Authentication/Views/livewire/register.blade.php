...
<div class="py-16 sm:py-24 bg-gray-50 flex items-center justify-center flex-grow">
    <div class="w-full max-w-md bg-white border border-gray-200 p-8 rounded-lg shadow-sm space-y-8 animate-fade-in" 
        ...
        <!-- Tabs Navigation (Only if both Google and Email registration are allowed) -->
        ...
        @if($login_google_enabled)
            @php
                $googleClientId = \App\Models\Setting::getValue('google_client_id', '');
            @endphp
            <div x-show="activeTab === 'google'" class="space-y-4">
                <!-- Google Sign Up Button -->
                @if(!empty($googleClientId))
                    <a href="{{ route('login.google') }}" class="w-full inline-flex items-center justify-center gap-3 rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition">
        ...
        <!-- Tab Content: Email & Password Register -->
        @if($login_email_enabled)
            <div x-show="activeTab === 'email'" class="space-y-4" style="display: none;">
                <form wire:submit.prevent="register" class="space-y-4">
                    <div>
                        <label for="name" class="block text-xxs font-bold uppercase tracking-wider text-gray-400">Full Name</label>
                        <input wire:model="name" type="text" id="name" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none bg-white">
                        @error('name') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="email_reg" class="block text-xxs font-bold uppercase tracking-wider text-gray-400">Email Address</label>
                        <input wire:model="email" type="email" id="email_reg" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none bg-white">
                        @error('email') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="password_reg" class="block text-xxs font-bold uppercase tracking-wider text-gray-400">Password</label>
                        <input wire:model="password" type="password" id="password_reg" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none bg-white">
                        @error('password') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="password_confirmation" class="block text-xxs font-bold uppercase tracking-wider text-gray-400">Confirm Password</label>
                        <input wire:model="password_confirmation" type="password" id="password_confirmation" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none bg-white">
