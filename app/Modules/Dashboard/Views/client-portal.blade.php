...
<head>
    ...
    <title>@yield('title', 'Client Portal') - {{ \App\Models\Setting::getValue('site_title', 'Metrica Polls') }}</title>
...
<body class="h-full font-sans antialiased {{ $theme === 'Dark' ? 'text-gray-100 bg-brand-navy' : ($theme === 'Glassmorphism' ? 'text-white bg-transparent' : 'text-gray-900 bg-gray-50') }}" x-data="{ mobileMenuOpen: false }">
    ...
    <aside class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-64 lg:flex-col border-r border-gray-200 bg-white">
        <div class="flex h-16 shrink-0 items-center border-b border-gray-100 px-6">
            <a href="{{ route('client.requests') }}" class="flex items-center gap-2">
                <img src="{{ asset('images/logo.png') }}" alt="Metrica Polls Logo" class="h-6 w-auto">
                <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-0.5 text-xxs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10">CLIENT</span>
            </a>
        </div>
            ...
            <ul role="list" class="flex flex-1 flex-col gap-y-7">
                <li>
                    <ul role="list" class="-mx-2 space-y-1">
                        <li>
                            <a href="{{ route('client.requests') }}" class="{{ request()->routeIs('client.requests') ? 'bg-gray-50 text-gray-900 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                Research Requests
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('client.requests.create') }}" class="{{ request()->routeIs('client.requests.create') ? 'bg-gray-50 text-gray-900 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                Request Research
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('client.projects') }}" class="{{ request()->routeIs('client.projects') ? 'bg-gray-50 text-gray-900 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                Active Projects
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('admin.reports') }}" class="{{ request()->routeIs('admin.reports') ? 'bg-gray-50 text-gray-900 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                ...
                <nav class="mt-6 flex flex-1 flex-col px-4">
                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                        <li>
                            <ul role="list" class="-mx-2 space-y-1">
                                <li>
                                    <a href="{{ route('client.requests') }}" class="text-gray-900 bg-gray-50 group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6">
                                        Research Requests
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('client.requests.create') }}" class="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                        Request Research
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('client.projects') }}" class="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                        Active Projects
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('admin.reports') }}" class="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex gap-x-3 rounded-md p-2 text-sm leading-6">
                                        Reports & AI Analytics
                                    </a>
                                </li>
                            </ul>
        ...
        <header class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
            ...
            <div class="flex-1 flex items-center text-sm font-medium text-gray-500">
                <span class="text-gray-950 font-bold">
                    {{ auth()->user()->clientOrganization->name ?? 'Individual Client Profile' }}
                </span>
                <span class="mx-2 text-gray-300">|</span>
                <span>@yield('page_title', 'Client Portal')</span>
            </div>
            ...
            <div class="flex items-center gap-x-4 lg:gap-x-6" x-data="{ open: false }">
                <div class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" class="absolute right-0 z-10 mt-2.5 w-48 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none" style="display: none;">
